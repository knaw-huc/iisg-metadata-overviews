...
os.mkdir(directory_converted)

# path to the .csv file in the ./transformed folder
transf_path = Path("transformed/archive.csv")

# read csv as dataframe
# low_memory=False is needed because DtypeWarning: Columns (0) have mixed types"
archives_df_v1 = pd.read_csv(transf_path, sep="\t", header=None, names=['id','marc_field','value'], compression='gzip', low_memory=False)
...
archives_df.to_csv('converted/archives_csv.csv', sep = '\t', index = False, compression = 'gzip')
