...
print("Note that this script takes approx. 5 minutes to run")
...
authority_df_v1 = pd.read_csv(f'{data_transformed_directory}/authority.gzip', sep="\t", header=None, names=['id','marc_field','value'], compression='gzip', low_memory=False)

# drop the first row because it contains the old column names
authority_df_v2 = authority_df_v1.iloc[1:]

# # to check counts before filling in empty values
# print(authority_df_v2.info(verbose = True, show_counts = True))
# print(authority_df_v2.head(100))

# make a copy
authority_df_v3 = authority_df_v2.reset_index(drop=True).copy()

# convert the transformed data into a csv where each column is a marc field/subfield
authority_df_v4 = authority_df_v3.pivot_table(index="id", columns="marc_field", values="value", aggfunc=lambda x: 'Ω'.join(x.dropna()))

# # to check counts before filling in empty values
# print(authority_df_v4.info(verbose = True, show_counts = True))

authority_df_v5 = authority_df_v4.fillna("null")

# # inspect the dataframe before storing it
# print(authority_df_v5.info(verbose = True, show_counts = True))
# print(authority_df_v5.describe())
# print(authority_df_v5.head(10))

# make a copy
authority_df = authority_df_v5.reset_index(drop=True).copy()

authority_df.to_csv(f'{data_converted_directory}/authority_as_csv.gzip', sep = '\t', index = False, compression = 'gzip')

# print(authority_df_v5.head(100))
